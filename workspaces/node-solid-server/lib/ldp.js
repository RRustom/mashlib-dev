...
class LDP {
  ...
  /*
    Read file from url, using resourceMapper to map url to filepath
   */
  ...
  /*
    TODO: list all the items in a container?
   */
  ...
  async put (url, stream, contentType) {
    ...
    // DEV: remove quota check for now
    ...
    const ipfs_path = url.url.replace("/", "")
    ...
    debug.handlers('PUT -- Wrote data to: ' + ipfs_path + ' with CID: ' + cid)

    const data = await all(node.cat(cid)) //await node.cat(cid) //uint8ArrayConcat(await all(node.cat(cid)))
