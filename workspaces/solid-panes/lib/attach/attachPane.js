...
module.exports = {
  ...
  render: function render(subject, context) {
    ...
    var getSortKeySimple = function getSortKeySimple(_c) {
      var uriMap = {
    ...
    var getMembersAndSort = function getMembersAndSort(subject) {
    ...
    var showFiltered = function showFiltered(mode) {
      ...
      function getFiltered() {
        ...
        }) : objects.filter(function (y) {
          return !!kb.holds(currentSubject, predicate, y);
        }) : objects.filter(function (y) {
          return kb.each(undefined, predicate, y).length === 0;
        });
      }
    };
    ...
    if (multi) {
      var subjectList = UI.widgets.selectorPanel(dom, kb, subject, predicate, false, subjects, options, showSubject, linkClicked);
