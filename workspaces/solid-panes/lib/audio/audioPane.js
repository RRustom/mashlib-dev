...
module.exports = {
  ...
  render: function render(subject, context) {
    ...
    var looksRedundant = function looksRedundant(x) {
    ...
    var guessNames = function guessNames(x) {
    ...
    var moveOn = function moveOn(current, level) {
      return new Promise(function (resolve) {
        level = level || 0;
        if (!options.chain) return resolve(null); // Ideally navigate graph else cheat with URI munging:
    ...
    var endedListener = function endedListener(event) {
      var current = kb.sym(event.target.getAttribute('src'));
      if (!options.chain) return;

      var tryNext = function tryNext(cur) {
        var current = cur;
