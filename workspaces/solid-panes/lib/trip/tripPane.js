...
module.exports = {
  ...
  label: function label(subject, context) {
    ...
    var t = kb.findTypeURIs(subject); // if (t['http://www.w3.org/2000/10/swap/pim/qif#Transaction']) return "$$";
  ...
  render: function render(subject, context) {
    ...
    var renderTrip = function renderTrip(subject, thisDiv) {
      ...
      query.pat.optional.push(opt); // opt = kb.formula();

      query.pat.add(v.transaction, UI.ns.qu('in_USD'), v.in_USD); // query.pat.optional.push(opt);

      var calculations = function calculations() {
        var total = {};
        var trans = kb.each(undefined, TRIP('trip'), subject); // complain("@@ Number of transactions in this trip: " + trans.length);

        trans.forEach(function (t) {
          var ty = kb.the(t, ns.rdf('type')); // complain(" -- one trans: "+t.uri + ' -> '+kb.any(t, UI.ns.qu('in_USD')));
          ...
          if (ty && ty.uri) {
            ...
            if (lit) {
              total[tyuri] = total[tyuri] + parseFloat(lit.value); // complain('      Trans type ='+ty+'; in_USD "' + lit
    ...
    if (UI.ns.qu('Transaction') in kb.findSuperClassesNT(subject)) {
      var ts = kb.each(undefined, ns.rdf('type'), subject);
      var triples = [];
      var index = [];
