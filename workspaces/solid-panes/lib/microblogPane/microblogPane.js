...
module.exports = {
  ...
  render: function render(s, doc) {
    ...
    var Microblog = function Microblog(kb) {
      this.kb = kb; // attempt to fetch user account from local preferences if just
    ...
    Microblog.prototype.generateNewMB = function (id, name, avatar, loc) {
      ...
      var cbgenUserMB = function cbgenUserMB(a, success, c, d) {
        if (success) {
          alert('Microblog generated at ' + host + '#' + id + 'please add <b>' + host + '</b> to your foaf.'); // mbCancelNewMB()   @@TBD
      ...
      var genUserMB = [// user
      new UI.rdf.Statement(kb.sym(host + '#' + id), RDF('type'), SIOC('User'), kb.sym(host)), new UI.rdf.Statement(kb.sym(host + '#' + id), SIOC('creator_of'), kb.sym(host + '#mb'), kb.sym(host)), new UI.rdf.Statement(kb.sym(host + '#' + id), SIOC('creator_of'), kb.sym(host + '#mbn'), kb.sym(host)), new UI.rdf.Statement(kb.sym(host + '#' + id), SIOC('creator_of'), kb.sym(host + '#fav'), kb.sym(host)), new UI.rdf.Statement(kb.sym(host + '#' + id), SIOC('name'), name, kb.sym(host)), new UI.rdf.Statement(kb.sym(host + '#' + id), SIOC('id'), id, kb.sym(host)), new UI.rdf.Statement(kb.sym(host + '#' + id), RDF('label'), id, kb.sym(host)), new UI.rdf.Statement(s, FOAF('holdsAccount'), kb.sym(host + '#' + id), kb.sym(host)), // microblog
      new UI.rdf.Statement(kb.sym(host + '#mb'), RDF('type'), SIOCt('Microblog'), kb.sym(host)), new UI.rdf.Statement(kb.sym(host + '#mb'), SIOC('has_creator'), kb.sym(host + '#' + id), kb.sym(host)), // notification microblog
      new UI.rdf.Statement(kb.sym(host + '#mbn'), RDF('type'), SIOCt('Microblog'), kb.sym(host)), new UI.rdf.Statement(kb.sym(host + '#mbn'), SIOC('topic'), kb.sym(host + '#' + id), kb.sym(host)), new UI.rdf.Statement(kb.sym(host + '#mbn'), SIOC('has_creator'), kb.sym(host + '#' + id), kb.sym(host)), // favorites container
    ...
    Pane.prototype.header = function (s, doc) {
      ...
      function lsFollowUser() {
        var myUser = kb.sym(mb.getMyURI()); // var Ifollow = that.Ifollow
          ...
          var _ret = function () {
            ...
            for (var word in words) {
              if (words[word].match(/@\w+/)) {
                ...
                recipient = myFollowList.selectUser(atUser);

                if (recipient[0] === true) {
                  meta.recipients.push(recipient[1][0]);
                } else if (recipient[1].length > 1) {
                  // if  multiple users allow the user to choose
                  xrecipients = doc.createElement('select');
                  xrecipientsSubmit = doc.createElement('input');
      ...
      headerContainer.className = 'header-container'; // ---create status update box---
      ...
      subheaderContainer.className = 'subheader-container'; // user header
      // this.creator
      ...
      for (var c in creators) {
        if (kb.whether(creators[c], RDF('type'), SIOC('User')) && kb.whether(kb.any(creators[c], SIOC('creator_of')), RDF('type'), SIOCt('Microblog'))) {
          var creator = creators[c]; // var mb = kb.sym(creator.uri.split("#")[0]);
      ...
      if (creator) {
        this.creator = mb.getUser(creator); // ---display avatar, if available ---
        ...
        subheaderContainer.appendChild(userName); // ---display follow button---
    ...
    Pane.prototype.generatePost = function (post, _me) {
      ...
      */
      ...
      var Post = mb.getPost(post); // username text
      ...
      xpostLink.appendChild(postLink); // LINK META DATA (MENTIONS, HASHTAGS, GROUPS)
      ...
      xpostContent.innerHTML = postText; // in reply to logic
      ...
      var mbDeletePost = function mbDeletePost(evt) {
        ...
        var reallyDelete = function reallyDelete() {
          ...
          var mbconfirmDeletePost = function mbconfirmDeletePost(a, success) {
            if (success) {
              that.notify('Post deleted.'); // update the ui to reflect model changes.
    ...
    Pane.prototype.streamView = function (s, doc) {
      ...
      if (mbPosts.length > 0) {
        var postList = this.generatePostList(mbPosts); // generate stream
    ...
    Pane.prototype.notificationsView = function (s, doc) {
      ...
      var mbnPosts = [];
      var mbmPosts = []; // get mbs that I am the creator of.
    ...
    var microblogpane = doc.createElement('div'); //      var getusersfollows = function(uri){
