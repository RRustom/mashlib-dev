    ...
    showMenu: function showMenu(e, menuType, inputQuery, extraInformation, _order) {
      ...
      switch (menuType) {
        ...
        case 'LimitedPredicateChoice':
          ...
          selectItem = function selectItem(e) {
            ...
            for (var _i = 0; _i < predicateChoices.length; _i++) {
              if (predicateChoices[_i].sameTerm(selectedPredicate)) {
                predicateChoices.unshift(predicateChoices.splice(_i, 1)[0]);
              }
            }
      ...
      switch (menuType) {
        case 'DidYouMeanDialog':
          ...
          var bnodeTerm = extraInformation.bnodeTerm; // have to do style instruction passing
        ...
        case 'PredicateAutoComplete':
          ...
          for (var _i2 = 0; _i2 < predicates.length; _i2++) {
            var tempQuery = {};
            tempQuery.vars = [];
            tempQuery.vars.push('Kenny');
            var tempBinding = {};
            tempBinding.Kenny = kb.fromNT(predicates[_i2].NT);
        ...
        case 'GeneralAutoComplete':
          ...
          for (var _i4 = 0; _i4 < entries.length && _i4 < 10; _i4++) {
            // do not show more than 30 items
            // console.log("\nGEN ENTRIES["+i+"] = "+entries[i]+"\n");//hq
            var thisNT = entries[_i4][1].toNT(); // console.log("thisNT="+thisNT+"\n");


            var tr = table.appendChild(myDocument.createElement('tr'));
            tr.setAttribute('about', thisNT);
            var th = tr.appendChild(myDocument.createElement('th'));
            th.appendChild(myDocument.createElement('div')).appendChild(myDocument.createTextNode(entries[_i4][0])); // var theTerm = entries[i][1]
        ...
        case 'JournalTitleAutoComplete':
          ...
          for (var _i5 = 0; _i5 < juris.length; _i5++) {
            var juri = juris[_i5];
        ...
        case 'LimitedPredicateChoice':
          ...
          for (var _i6 = 0; _i6 < choices.length; _i6++) {
            addMenuItem(choices[_i6]);
          }
        ...
        default:
          ...
          switch (inputQuery.constructor.name) {
            case 'Array':
              for (var _i7 = 0; _i7 < inputQuery.length; _i7++) {
                kb.query(inputQuery[_i7], addPredicateChoice(inputQuery[_i7]), nullFetcher);
              }
