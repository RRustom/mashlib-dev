...
module.exports = {
  makeQueryRow: makeQueryRow,
  QuerySource: QuerySource,
  viewAndSaveQuery: viewAndSaveQuery // Main function to generate and use the query
...
function makeQueryRow(q, tr, constraint) {
  var kb = UI.store; // predtr = predParentOf(tr)
  // var nodes = tr.childNodes
  // var n = tr.childNodes.length

  var inverse = tr.AJAR_inverse; // var hasVar = 0
  ...
  var predtr = predParentOf(tr); // /////OPTIONAL KLUDGE///////////
  ...
  var v = tr.AJAR_variable ? tr.AJAR_variable : kb.variable(UI.utils.newVariableName());
  q.vars.push(v);
  v.label = hasParent ? parentVar.label : UI.utils.label(parentVar);
  v.label += ' ' + UI.utils.predicateLabelForXML(st.predicate, inverse);
  var pattern = makeRDFStatement(v, parentVar); // alert(pattern)
  ...
  tr.AJAR_variable = v; // UI.log.debug('Final variable: '+tr.AJAR_variable)
...
function saveQuery(selection, qs) {
  ...
  function resetOutliner(pat) {
    ...
    for (var _i = 0; _i < n; _i++) {
      ...
      tr = pattern.tr; // UI.log.debug('tr: ' + tr.AJAR_statement);
  ...
  resetOutliner(q.pat); // NextVariable=0;
