...
module.exports = {
  ...
  render: function render(subject, context) {
    ...
    div.setAttribute('style', '  border-top: solid 1px #777; border-bottom: solid 1px #777; margin-top: 0.5em; margin-bottom: 0.5em '); // If this is a class, look for all both explicit and implicit
    ...
    if (sts.length > 0) {
      ...
      if (subject.sameTerm(ns.rdf('Property'))) {// / Do not find all properties used as properties .. unless look at kb index
      ...
      if (sts.length > 10) {
        ...
        tr.appendChild(dom.createTextNode('' + sts.length)); // tr.AJAR_statement=sts[i]
